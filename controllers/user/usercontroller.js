...
const loadLandingPage = async (req, res) => {
  try {
    ...
    // Fetch available brands with images
    const brands = await Brand.find({ isBlocked: false }).select('brandName brandImage').lean();

    // ✅ Render with user, products, and brands
    return res.render("user/landingpage", { user: userData, products, brands });
  } catch (err) {
